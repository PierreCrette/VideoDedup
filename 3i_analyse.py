...
def calcfp(file, quality, display=False):
  ...
  if os.path.exists(file):
    ...
    if os.path.splitext(file)[1] == '.jpg':
      ...
      if quality == 3:
          #s = 'convert "' + file + '-size 320x180 -crop 200x160+60+10 +repage -modulate 100,0 -blur 3x99 -normalize -equalize -resize 50x40 -threshold 50% ' + tmpfile +'"'
      ...
      if err == None:
        if os.path.exists(tmpfile):
          ...
          log('ERROR in calcfp - key file generated but not exist: ' + tmpfile + ' (from (' + file + ')', 1)
  return result
...
def loadunwanted(folder, action):
    ...
    #    log('len(unwanted)=' + str(len(unwanted)) + ', len(unwanted0)=' + str(len(unwanted0)))

    if action == 'ctrl':
        for sl in srclst:
            srclst2.append(sl[0])
        #srclst2 = sorted(srclst2)
        for file in os.listdir(folder):
            if (os.path.splitext(file)[1] == '.txt'):
                f = open(folder + file, 'r')
                nok = False
...
def mp_HD_function(imagefile):
    return([MidName(imagefile),calcfp(imagefile,3)])
...
if len(sys.argv)<3:
    ...
    if folderimgraw[-1] != "/":
        ...
        foutputhd = folderimgraw + "/hddb_3h.fp"
    else:
        folderimg = folderimgraw + "db/"
        folderana = folderimgraw + "ana-" + env + "-not-saved/"
        foutputhd = folderimgraw + "hddb_3h.fp"
    ...
    sshell = sshell + ' -ctrlref=' + str(ctrlref) + ' -out=' + foutput + ' -hdout=' + foutputhd
    if fake:
      sshell = sshell + ' -fake'
    ...
    #        ' dupes found. {:_}'.format(nbstill) + ' stills rejected and {:_}'.format(nbunwant) + ' unwanted images and {:_}'.format(nbunwantth) + ' similar to uw images.', 1)
        ...
        for line in f:
            if (nbline % 1000000 == 0) or (nbline == 100000) or (nbline == 10000) or (nbline == 1000):
                log(duration(time.perf_counter() - perf) + ' - Loading file and first checks, line {:_}'.format(nbline) + '/{:_}'.format(nblines) + ', t2=' + duration(t2, False) + ', t3=' + duration(t3, False) , 1)
            ...
            if testdiff and (line[:5] == 'file='):
                ...
                if src not in srclst2:
                    log(src + ' referenced but not on disk. Remove from dataset.', 2)
                    testdiff = False
                else:
            ...
            if testdiff and (line[:4] == 'key='):
                ...
                if testdiff and (thresholduw > 0):
                    ...
                    #log('len(unwanted)=' + str(len(unwanted)))
                    iuw = 0
                    nuw = len(unwanted)
                    while testdiff and (iuw < nuw):
                      testdiff = (gmpy2.hamdist(key,unwanted[iuw]) > thresholduw)
                      iuw = iuw + 1
                    if not(testdiff):
                      nbunwantth = nbunwantth + 1
                    t3 = t3 + time.perf_counter() - perf3
    
        f.close
        worksize = len(resultsetvideo)
        log(duration(time.perf_counter() - perf) + ' - STEP1 done. On {:_}'.format(nblines // 6) + ' duplicates images, ' + txtgreen + '{:_}'.format(worksize) + ' ({:_}%)'.format(600 * worksize // nblines) + txtnocolor + \
            ' dupes found. {:_}'.format(nbstill) + ' stills rejected, {:_}'.format(nbunwant) + ' unwanted images,  {:_}'.format(nbunwantth) + ' images distant from a unwanted <= ' + str(thresholduw) + \
            ' and {:_}'.format(nbdist) + ' images with distance > ' + str(maxdiff), 1)
    #    log(duration(time.perf_counter() - perf) + ' - STEP1 done. On {:.{prec}f}'.format(nblines/6, prec=0) + ' duplicates images, ' + txtgreen + '{:_}'.format(worksize) + txtnocolor + \
    #        ' dupes found. {:_}'.format(nbstill) + ' stills rejected and {:_}'.format(nbunwant) + ' unwanted images and {:_}'.format(nbunwantth) + ' similar to uw images.', 1)
        log('     ' + duration(t2, False) + ' - t2 = Uwanted images detection.')
        log('     ' + duration(t3, False) + ' - t3 = Images with distance to uw image <= ' + str(thresholduw))
    ...
    #                            log(rsv[i][2][k], 3)
      ...
      #            print(srclst[0][0] + ' =? ' + rsv[i][1][0])
          ...
          if keep:
            log('{:_}'.format(rejthr) + ' + {:_}'.format(rejimg) + ' rejections due to common images < {:_}'.format(threshold) + ', {:_}'.format(rejdel) + ' deleted sources.', 1)
          else:
    ...
    #          print(rsvelt)
          if skiphd:
            log(duration(time.perf_counter() - perf) + ' - Skip HD control.')
          else:
    ...
    # Complete HD fingerprint database  
            log(duration(time.perf_counter() - perf) + ' - HD fingerprint to complete in multi processing mode.', 1)
            mpdata = []
            f = open(foutputhd, 'a')
            
            jump = 10
            if len(resultsetvideo) > 500:  jump = 20
            if len(resultsetvideo) > 2000: jump = 50
            if len(resultsetvideo) > 5000: jump = 100
            ...
            for i in range(len(resultsetvideo)):
              ...
              for j in range(len(resultsetvideo[i][2])):
                ...
                try:
                  ...
                  if imagefile != '':
                    mpdata.append(imagefile)
                  else:
                    log('WARNING : cannot create HD fp of ' + resultsetvideo[i][2][j] + ' cause path not in initial load.', 2)
              results = pool.map(mp_HD_function, mpdata)
              mpdata = []
    #          print(results)
              for r in results:
                hdcacheimg.append(r[0])
                hdcachekey.append(r[1])
    ...
    # HD comparison
        ...
        #    resultsetvideo.append([1, setvideo, setimg, setprt])
            ...
            rsv = []
            for i in range(len(resultsetvideo)):
                if ((time.perf_counter() - perf1) > 10) and (i > 0):
                ...
                for j in range(len(resultsetvideo[i][2])):
                  ...
                  try:
                    ...
                    imagefile = newimage(resultsetvideo[i][2][j])  #Path of image found in memory ?
                    if imagefile != '':
                      log('ERROR HD fingerprint does not exist. Continue in single process mode. You can kill and relaunch if too long.', 0)
