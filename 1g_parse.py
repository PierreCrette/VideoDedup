...
def SameNames():
  ...
  for i in range(len(srclst)):
    ...
    if srclst[i][0] == prev[0]:
      ...
      if not(firstrenamed):
        log(duration(time.perf_counter() - perf) + ' - File ' + prev[1] + prev[0] + ' is referenced multiples times. Renaming all of them.', 1)
...
def mpffmpeg(elt):
  ...
  if line != pid:
    ...
    log(duration(time.perf_counter() - perf) + '    MP --- .run flag for ' + mpfolderv + mpfile + ' Skip due to parallel mode ', 0)
    log(duration(time.perf_counter() - perf) + '    MP ---------------------------------------------------------------------------------------------------', 0)
  else:
    #Call ffmpeg
...
#    try:
  ...
  if (todo and parallel):
    if os.path.exists(folderi2):
      shutil.rmtree(folderi2)
    mpdata.append([folderi, folderi2, fvideo, file, args, s, cptdone, cpttodo])
    ...
    if os.path.isdir(folderv):
        ...
        for file in os.listdir(folderv):
            ...
            if os.path.isdir(folderv+file):
                ...
                if (len(mpdata) > 10 * threads) or ((len(mpdata) > 0) and (cpttodo == cptdone)):
                  if clean:
                    log('ERROR: Should not be executed in clean mode.')
                  time.sleep(3)
                  result = poolffmpeg.map(mpffmpeg, mpdata)
...
if len(sys.argv)<2:
    ...
    poolfp = multiprocessing.Pool(threads)
    poolffmpeg = multiprocessing.Pool(threads)
