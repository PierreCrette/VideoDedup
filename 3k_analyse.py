...
def mp5_HD_DistanceControl(mpinput):
  ...
  rsvuniti = resultsetvideo[mpinput]
  hdkey  = []
  ...
  if (hdbest != []):
    ...
    if len(hdbest2) <  threshold:
      ...
      if (nbia < threshold) or (nbib < threshold):
        log(duration(time.perf_counter() - perf) + ' - Resultset rejected after HD control. nbia={} and nbiab={}'.format(nbia,nbib,len(hdbest2)) + ' Best distances = ' + s, 3)
      else:
        mp5result = [i,hdbest2]
...
sshell = sshell + ' -t=' + str(threshold)  + ' -tu=' + str(thresholduw) + ' -tuwclean=' + str(tuwclean) + ' -maxdiff=' + str(maxdiff) + ' -hdmaxdiff=' + str(hdmaxdiff)
...
log(duration(time.perf_counter() - perf) + ' - Create a pool of ' + str(threads) + ' threads.')
...
if contuwfp:
    ...
    if worktodo > 0:
      ...
      s = s + ' ({:_}%)'.format(100 * worksize // nbline) + txtnocolor +  ' dupes found. {:_}'.format(nbstill) + ' stills rejected, {:_}'.format(nbunwant)
        ...
        #Calculate HD distance to limit resultset
          ...
          for r in mpdatalist:
            if r != prev:
              tmprs.append(r)
            prev = r
          log(duration(time.perf_counter() - perf) + ' - For {:_} images to compare in HD, '.format(len(mpdatalist)) + '{:_} uniques references.'.format(len(tmprs)), 1)
